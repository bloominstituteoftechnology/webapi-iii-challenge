...
server.post('/api/users', formatUserName, (req, res) => {
  ...
  if (req.body.name > 128) { return res.json({ message: `go shorter on name` }) }
...
server.put('/api/users/:id', formatUserName, (req, res) => {
  ...
  userDB.update(req.params.id, req.body)
    .then(count => res.json({ message: `Way to go, returned count ${count}` }))
    .catch(err => console.log(err));
});

server.delete('/api/users/:id', (req, res) => {
  userDB.remove(req.params.id)
    .then(removed => res.json({ message: `Way to go, removed count ${removed}` }))
    .catch(err => console.log(err));
})
