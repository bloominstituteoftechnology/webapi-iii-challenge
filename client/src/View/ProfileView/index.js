      ...
      <Container>
        <Row>
          <Col>
            ...
            {this.props.fetchedUser ? (
              <h1>
                Welcome To {this.props.currentProfileView.user.name}&apos;s
                Profile!
              </h1>
            ) : null}
          </Col>
        ...
        <Row>
          <Col md={{ size: "6", offset: "3" }}>
            ...
            this.props.fetchedUserPosts ? (
              <Alert
                color="danger"
                style={{ fontSize: "24px", margin: "20px" }}
              >
                <p>No posts {this.props.currentProfileView.user.name}</p>
                <Button color="success">Write Post</Button>
              </Alert>
            ) : null}
            {this.props.fetchedUserPosts
              ? this.props.currentUserPosts.map(post => {
                  if (post) {
                    return <PostCard key={post.id + Math.random()} {...post} />;
                  } else {
                    return null;
                  }
                })
