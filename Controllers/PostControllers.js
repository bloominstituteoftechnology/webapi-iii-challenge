...
const PostController = {
	getPost: (req, res) => {
		...
		PostHelpers.get(id)
			...
			.catch(err => res.status(404).json({error: 'Cannot find post'}));
	},
	getPostTags: (req, res) => {
		const { id } = req.params;
		PostHelpers.get(id)
	...
	createPost: (req, res) => {
		...
		if('text' in post && 'postedBy' in post) {
			PostHelpers.insert(post)
				.then(id => {
					return res.status(201).json({success: `post with the id: ${id.id} was created`});
				})
				.catch(err => res.status(500).json({error: 'could not save new post'}));
		} else {
	...
	updatePost: (req, res) => {
		...
		if('text' in post && 'postedBy' in post) {
			PostHelpers.update(id, post)
				...
				.catch(err => res.status(404).json({error: `could not update ${id}`}));
		} else {
	...
	removePost: (req, res) => {
		...
		PostHelpers.remove(id)
			.then(() => res.status(200).json({success: `removed post with the id of ${id}`}))
			.catch(err => res.status(500).json({error: 'something went wrong'}));
	}
}
