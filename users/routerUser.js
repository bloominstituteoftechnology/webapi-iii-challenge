  ...
  db
    ...
    .then(newID => {
      db
        ...
        .catch(error =>
          res.status(500).json({ error: "Added user could not be retrieved." })
        );
    })
    .catch(error =>
      res.status(500).json({ error: "User could not be added to database." })
    );
});
...
router.put("/:id", (req, res) => {
  ...
  db
    ...
    .catch(error =>
      res
        .status(500)
        .json({ error: `user with id ${id} could not be updated.` })
    );
});
